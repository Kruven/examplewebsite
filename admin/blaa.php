...
for ($x = 0; $x < sizeof($mainDirectory->folderNames); $x++) {
    ...
    echo $mainDirectory->directory;
    echo '<br>';
    echo $mainDirectory->folderNames[$x];
    echo '<br>';
...
function setFoldersContents($folder, $directory) {
    // Get all the folders
    $folderDirectories = array();
    $folderDirectories = glob($directory."*", GLOB_ONLYDIR);
    $folderNames = array();
    for ($x = 0; $x < sizeof($folderDirectories); $x++) {
        $directories = explode("/", $folderDirectories[$x]);
        echo 'FOLDER:'.$folderDirectories[$x].'END<br>';
    ...
    $files = array();
    for ($x = 0; $x < sizeof($fileDirectories); $x++) {
        $directories = explode("/", $fileDirectories[$x]);
        echo 'FILE: '.$fileDirectories[$x].'<br>';
        $files[] = $directories[sizeof($directories) - 1];
    }

    // Save the data
    for ($x = 0; $x < sizeof($folderDirectories); $x++) {
        echo 'DIRECTORY:'.$folderDirectories[$x];
    ...
    for ($x = 0; $x < sizeof($folder->folders); $x++) {
        echo $x;
        if ($folder->folderNames[$x] != NULL) {
            setFoldersContents($folder->folders[$x], $directory . $folder->folderNames[$x] . "/");
        }
    }
}

/* Checks to se if a string ends with another string
